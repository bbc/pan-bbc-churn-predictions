...
def build(target, train_features, X_pub, y_pub, my_skf):
    ...
    def behaviour_feature(element):
        return 'sounds_device_count' in element or \
               ('device_sounds_st_' in element and 'perc' in element) or \
               'device_sounds_st_preferred' in element or \
               'sounds_tod_' in element or \
               'sounds_dow_' in element or \
    ...
    def crosssell_feature(element):
        return ('freq_seg_' in element and 'sounds' not in element and 'panbbc' not in element)
    ...
    def producttype_feature(element):
        return ('sounds_prod_' in element and 'perc' in element) or \
            ('sounds_prod_' in element and 'preferred' in element)

    producttype_lgbm = stackableLGBMClassifier(
        name = 'Product Type',
        target = target,
        features = [f for f in train_features if producttype_feature(f)],
        objective = 'binary',
        boosting_type = 'gbdt',
        num_leaves = 31,
        n_estimators = 100,
