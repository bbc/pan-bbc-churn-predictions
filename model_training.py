...
impute_strategies = pd.DataFrame( {
    'colname': ['profile_age', 'profile_nation', 'profile_barb_region', 'profile_acorn_type_description',
                'profile_acorn_group_description', 'profile_acorn_category_description', 'profile_gender_enriched'],
    'strategy': ['regression', 'classification', 'classification', 'classification', 'classification', 'classification',
                 'classification', ]
} )

helper_features = ['ew_' + str( x ) for x in range( 2, 15 )] + \
...
fig.savefig( './charts/perf/lastrun_holdout_ROC' )
