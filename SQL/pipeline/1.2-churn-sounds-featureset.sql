DROP TABLE IF EXISTS central_insights_sandbox.ap_churn_sounds_feature_set;
CREATE TABLE central_insights_sandbox.ap_churn_sounds_feature_set AS
SELECT
...
--   demos.gender_enriched_missing_flag as profile_gender_enriched_missing_flag,
  ...
  genre_aff.genre_distinct_count,
  ...
  devices.sounds_device_count,
  devices.iplayer_device_count,
	devices.sounds_st_desktop_web_perc as device_sounds_st_desktop_web_perc,
	devices.sounds_st_mobile_web_perc as device_sounds_st_mobile_web_perc,
	devices.sounds_st_app_perc as device_sounds_st_app_perc,
	devices.sounds_st_smart_tv_perc as device_sounds_st_smart_tv_perc,
	devices.sounds_st_other_web_perc as device_sounds_st_other_web_perc,
	devices.sounds_ev_desktop_web_perc as device_sounds_ev_desktop_web_perc,
	devices.sounds_ev_mobile_web_perc as device_sounds_ev_mobile_web_perc,
	devices.sounds_ev_app_perc as device_sounds_ev_app_perc,
	devices.sounds_ev_smart_tv_perc as device_sounds_ev_smart_tv_perc,
	devices.sounds_ev_other_web_perc as device_sounds_ev_other_web_perc,
...
--   iplayer_acti.iplayer_activating_brand as iplayer_activating_masterbrand,
  ...
  timeandday.tod_breakfast as sounds_tod_breakfast,
  timeandday.tod_daytime as sounds_tod_daytime,
  timeandday.tod_earlypeak as sounds_tod_earlypeak,
  timeandday.tod_peak as sounds_tod_peak,
  timeandday.tod_late as sounds_tod_late,
  timeandday.tod_overnight as sounds_tod_overnight,
  timeandday.dow_monday as sounds_dow_monday,
  timeandday.dow_tuesday as sounds_dow_tuesday,
  timeandday.dow_wednesday as sounds_dow_wednesday,
  timeandday.dow_thursday as sounds_dow_thursday,
  timeandday.dow_friday as sounds_dow_friday,
  timeandday.dow_saturday as sounds_dow_saturday,
  timeandday.dow_sunday as sounds_dow_sunday
...
FROM central_insights_sandbox.ap_churn_target targ
     ...
     AND targ.fresh = mkt_activity.fresh
       LEFT JOIN
     central_insights_sandbox.ap_churn_sounds_subscribes sounds_subscribes
     ON targ.bbc_hid3 = sounds_subscribes.bbc_hid3
     AND targ.fresh = sounds_subscribes.fresh
       LEFT JOIN
     central_insights_sandbox.ap_churn_sounds_bookmarks sounds_bookmarks
    ...
    and timeandday.destination = 'sounds'
WHERE targ.destination = 'sounds'
;
GRANT ALL ON central_insights_sandbox.ap_churn_sounds_feature_set TO GROUP central_insights;

DROP TABLE IF EXISTS central_insights_sandbox.ap_churn_sounds_training_sample;
CREATE TABLE central_insights_sandbox.ap_churn_sounds_training_sample AS
SELECT * FROM central_insights_sandbox.ap_churn_sounds_feature_set
WHERE fresh = 0
  and sounds_active_last_week = 1
ORDER BY random()
LIMIT 10000
;
GRANT ALL ON central_insights_sandbox.ap_churn_sounds_training_sample TO GROUP central_insights;

DROP TABLE IF EXISTS central_insights_sandbox.ap_churn_sounds_score_sample;
CREATE TABLE central_insights_sandbox.ap_churn_sounds_score_sample AS
SELECT * FROM central_insights_sandbox.ap_churn_sounds_feature_set
WHERE fresh = 1
  AND sounds_active_last_week = 1
ORDER BY random()
LIMIT 10000
;
GRANT ALL ON central_insights_sandbox.ap_churn_sounds_score_sample TO GROUP central_insights;

--duplicates
-- select count(*) from (
--                        select bbc_hid3, count(*) as count
--                        from central_insights_sandbox.ap_churn_sounds_feature_set
--                        group by 1
--                        having count > 1
--                      ) dupes;
--target: 0
